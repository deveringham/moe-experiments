...
#    target_q: requested activation frequencies (n_experts, n_layers)
# Returns array of prompts from results of length l
#    and the obtained q (n_experts, n_layers)
def construct_workload_qs(results, l, target_q, verbose=False, max_repeats=0):
    ...
    current_sum = torch.zeros_like(target_q)
...
def evaluate_workload_quality_qs(target_q, obtained_q):

    diff = obtained_q - target_q
...
def construct_workload_cvs(results, l, target_cvs, verbose=False, max_repeats=0):
    ...
    n_samples = len(results)
    k = results[0]['active_experts'].shape[1]
    #n_tokens = torch.stack([r['probs'].shape[0] for r in results])
...
def workload_sweep_cvs(results, target_cvs_list, target_ls, max_repeats=0, verbose=False):
    ...
    for l in target_ls:
        if verbose:
            print(f'Length: {l} prompts.')
        workloads[l] = {}
        for target_cvs in target_cvs_list:
            workload = {}
            p, cvs, indices = construct_workload_cvs(results, l, target_cvs, max_repeats=max_repeats)
            ...
            workloads[l][target_cvs] = workload
    if verbose:
        print('done!')
    return workloads
...
def plot_workload_sweep_cvs(workloads, alphas, cv_nat, title):
    ...
    for i, l in enumerate(target_ls):
        ...
        ax.plot(x, y, marker='^', label=f'Workload length: {l}', color=cv_colors[i])
    
    ax.set_title('Alpha vs. Unique Prompts')
    ax.set_xlabel('Target Alpha (CV Target Scaling)')
    ax.set_ylabel('% Unique Prompts')
    ax.set_xticks(target_alphas)
    ax.legend()
